...
Read and write Go library for [Electron ASAR](https://github.com/electron/asar) archives.

An ASAR file is an 8-byte Chromium pickle, a JSON index pickle, and concatenated
uncompressed file bytes. `Open` returns `io.SectionReader` values over packed files.
`Pack` and `WriteArchive` build the same layout, including shared offsets for
identical contents and SHA-256 integrity blocks. Hashes are not verified on read.
